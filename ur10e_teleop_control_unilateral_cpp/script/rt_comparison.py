...
def find_benchmark():
    ...
    try:
        prefix = subprocess.check_output(
            ['ros2', 'pkg', 'prefix', 'ur10e_teleop_control_unilateral_cpp'],
            stderr=subprocess.DEVNULL, text=True).strip()
        candidates.append(os.path.join(prefix, 'lib', 'ur10e_teleop_control_unilateral_cpp',
                                        'jitter_benchmark'))
    except Exception:
        pass
    candidates += [
        os.path.expanduser('~/colcon_ws/install/ur10e_teleop_control_unilateral_cpp/'
                            'lib/ur10e_teleop_control_unilateral_cpp/jitter_benchmark'),
...
def main():
    ...
    if not binary or not os.path.isfile(binary):
        print(f"jitter_benchmark not found. Build first: "
              f"colcon build --packages-select ur10e_teleop_control_unilateral_cpp")
        return 1
