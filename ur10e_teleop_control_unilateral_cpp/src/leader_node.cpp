...
void LeaderNode::control_loop() {
  ...
  while (running_) {
    ...
    // _unilateral: no backward haptic path. Leader doesn't use peer_q
    // for position coupling, so the mirror step is a no-op here.
    (void)raw_peer_q; (void)raw_peer_dq; (void)bilateral_active;

    // ---- control law ----
    std::array<double, 6> tau{};
    // Firmware handles gravity + friction comp; PC-side compensation would
    // be redundant on UR (unlike openarm). Leader is passive body + light
    // damping only.
    ...
    switch (cur_state) {
      case MODE_ACTIVE: {
        ...
        for (int i = 0; i < 6; ++i) {
          // Optional user-spring (zero in default config). No bilateral
          // coupling, no F/T feedback — follower tracks leader one way.
          double tau_user = cfg_.leader_kp_user[i] * (q_user[i] - q[i])
                          - cfg_.leader_kd_user[i] * dq[i];
          double tau_damp = -cfg_.leader_kd_hold[i] * dq[i];
          tau[i] = tau_user + ramp * tau_damp;
        }
        break;
      }
      case MODE_PAUSED:
