...
#include "ur10e_teleop_control_unilateral_cpp/leader_node.hpp"
...
int main(int argc, char** argv) {
  ...
  ur10e_teleop_control_unilateral_cpp::LeaderNode::Options opts;
  ...
  if (opts.use_rt) {
    ur10e_teleop_control_unilateral_cpp::lock_process_memory();
  }

  auto node = std::make_shared<ur10e_teleop_control_unilateral_cpp::LeaderNode>(opts);
  node->run();
  rclcpp::spin(node);
  node->stop();
  rclcpp::shutdown();
