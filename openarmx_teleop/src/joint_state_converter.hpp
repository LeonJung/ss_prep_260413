...
public:
    ...
    // [openarmx port] Robstride motor frame is inverted vs the URDF joint frame:
    // our driver uses direction_multipliers = -1 for all 7 arm joints (v10_simple_hardware).
    // Apply -1 to pos/vel/eff in BOTH directions so KDL gravity (joint frame) and the MIT
    // command (motor frame) stay consistent. Verified: with this the effective joint law is
    // tau_joint = Kp(q_peer - q) + Kd(qd_peer - qd) + tau_ff.  (arm; gripper handled separately)
    static constexpr double ARM_SIGN = -1.0;

    std::vector<JointState> motor_to_joint(const std::vector<MotorState>& m) const override {
        std::vector<JointState> j(m.size());
        for (size_t i = 0; i < m.size(); ++i) {
            j[i] = {ARM_SIGN * m[i].position, ARM_SIGN * m[i].velocity, ARM_SIGN * m[i].effort};
        }
        return j;
    }
    ...
    std::vector<MotorState> joint_to_motor(const std::vector<JointState>& j) const override {
        ...
        for (size_t i = 0; i < j.size(); ++i)
            m[i] = {ARM_SIGN * j[i].position, ARM_SIGN * j[i].velocity, ARM_SIGN * j[i].effort};
        return m;
    }
