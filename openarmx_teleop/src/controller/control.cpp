...
Control::Control(openarmx::can::socket::OpenArmX* arm, Dynamics* dynamics_l, Dynamics* dynamics_f,
...
bool Control::bilateral_step() {
    ...
    // [openarmx port] G_SCALE (0.93) trims gravity over-compensation (float-up) on our robot.
    for (size_t i = 0; i < arm_dof; i++) {
        joint_arm_states_ref[i].effort = G_SCALE * gravity[i] + friction[i];
    }
    ...
    // kp kd q dq tau
    std::vector<openarmx::robstride_motor::MotionControlParam> arm_cmds;
    arm_cmds.reserve(arm_dof);
    for (size_t i = 0; i < arm_dof; ++i) {
        arm_cmds.emplace_back(openarmx::robstride_motor::MotionControlParam{
    ...
    for (size_t i = 0; i < gripper_dof; ++i) {
        gripper_cmds.emplace_back(openarmx::robstride_motor::MotionControlParam{
            Kp_[i + arm_dof], Kd_[i + arm_dof], motor_gripper_states[i].position,
            motor_gripper_states[i].velocity, motor_gripper_states[i].effort});
    }

    // send command to arm
    openarm_->get_arm().send_motion_control_commands(arm_cmds);
    // send command to gripper
    openarm_->get_gripper().send_motion_control_commands(gripper_cmds);
    ...
    if (role_ == ROLE_LEADER) {
        ...
        for (size_t i = 0; i < arm_dof; ++i) {
            arm_cmds.emplace_back(
                openarmx::robstride_motor::MotionControlParam{0.0, 0.0, 0.0, 0.0, motor_arm_states[i].effort});
        }
        ...
        std::vector<openarmx::robstride_motor::MotionControlParam> hand_cmds;
        hand_cmds.reserve(hand_motor_refs.size());
        for (size_t i = 0; i < hand_motor_refs.size(); ++i) {
            hand_cmds.emplace_back(openarmx::robstride_motor::MotionControlParam{
                Kp_[i + arm_dof], Kd_[i + arm_dof], hand_motor_refs[i].position,
                hand_motor_refs[i].velocity, 0.0});
        }

        openarm_->get_arm().send_motion_control_commands(arm_cmds);
        openarm_->get_gripper().send_motion_control_commands(hand_cmds);
...
bool Control::AdjustPosition(void) {
    ...
    for (int step = 0; step < nstep; ++step) {
        ...
        arm_cmds.reserve(arm_motor_refs.size());
        for (size_t i = 0; i < arm_motor_refs.size(); ++i) {
            arm_cmds.emplace_back(openarmx::robstride_motor::MotionControlParam{kp_arm_temp[i], kd_arm_temp[i],
