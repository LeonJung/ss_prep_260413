...
bool LeaderNode::connect_robot() {
  ...
  cp.Kf_self = vec6_to_eigen(cfg_.hybrid_leader_kf);
  cp.Kf_peer = vec6_to_eigen(cfg_.hybrid_leader_kf_peer);
...
void LeaderNode::peer_cb(const sensor_msgs::msg::JointState::SharedPtr msg) {
  ...
  // _ff: follower puts tau_contact = J^T·F_TCP in the effort field.
