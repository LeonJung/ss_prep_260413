...
#include "ur10e_teleop_control_hybrid_cpp/leader_node.hpp"
...
int main(int argc, char** argv) {
  ...
  ur10e_teleop_control_hybrid_cpp::LeaderNode::Options opts;
  ...
  auto node = std::make_shared<ur10e_teleop_control_hybrid_cpp::LeaderNode>(opts);
  node->run();
  rclcpp::spin(node);
  node->stop();
  rclcpp::shutdown();
