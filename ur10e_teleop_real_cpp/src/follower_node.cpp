  ...
  while (running_) {
    ...
    switch (cur_state) {
      ...
      case /*MODE_PAUSED=*/1:
        ...
        break;
      case /*MODE_HOMING=*/2: {
        // Position-control homing via MODE_SERVOJ. Ramp from q_home_start
        // (captured on transition) to home_qpos_ over homing_duration
        // using a quintic ease profile — commanding home_qpos_ directly
        // exceeds UR's per-cycle joint-velocity limit.
