...
#include "ur10e_teleop_unilateral_vive_cpp/follower_node.hpp"
...
int main(int argc, char** argv) {
  ...
  ur10e_teleop_unilateral_vive_cpp::FollowerNode::Options opts;
  ...
  auto node = std::make_shared<ur10e_teleop_unilateral_vive_cpp::FollowerNode>(opts);
  node->run();
  rclcpp::spin(node);
  node->stop();
  rclcpp::shutdown();
