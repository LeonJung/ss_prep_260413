#!/usr/bin/env bash
# install.sh — Install dependencies for ur10e_teleop_unilateral_vive_cpp.
#
...
echo "  cd ~/colcon_ws && colcon build --packages-select ur10e_teleop_unilateral_vive_cpp"
echo "  source install/setup.bash"
echo ""
echo "=== Grant RT capabilities (one-time per rebuild) ==="
echo "  bash src/ur10e_teleop_unilateral_vive_cpp/script/setcap_rt.sh"
echo ""
echo "=== Jitter benchmark ==="
echo "  python3 src/ur10e_teleop_unilateral_vive_cpp/script/rt_comparison.py --duration 30 --rt-cpu 2 --load"
echo ""
echo "=== Real bilateral ==="
echo "  ros2 launch ur10e_teleop_unilateral_vive_cpp teleop_real.launch.py leader_rt:=true follower_rt:=true"
