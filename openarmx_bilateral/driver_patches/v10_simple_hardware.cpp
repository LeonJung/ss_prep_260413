...
hardware_interface::return_type OpenArmX_v10HW::read(
  ...
  // LATENCY LEVER (pipelined CAN I/O): the separate refresh_all() status-request round-trip
  // is redundant — every MIT command in write() already returns pos/vel/torque feedback.
  // So recv_all() here just drains the feedback the PREVIOUS write() elicited (buffered
  // during the read->update->write gap). Combined with dropping write()'s recv_all(), this
  // halves per-cycle CAN round-trips (was read=refresh+recv & write=send+recv -> now
  // read=recv & write=send). State is ~1 cycle stale (acceptable for teleop).
  // To REVERT: uncomment refresh_all() below AND recv_all(1000) at the end of write().
  // openarmx->refresh_all();
  openarmx->recv_all();
  ...
  if (hand_ && joint_names_.size() > ARM_DOF) {
    ...
    if (!gripper_motors.empty()) {
      ...
      // Gripper velocity (motor frame) — needed by friction comp (Fc*tanh(0.1*k*w));
      // was hardcoded 0 so the gripper friction term was always 0. torque still unmapped.
...
hardware_interface::return_type OpenArmX_v10HW::write(
  ...
  if (control_mode_ == ControlMode::MIT) {
    ...
    if (hand_ && joint_names_.size() > ARM_DOF) {
      ...
      // Feed the gripper effort command interface (was hardcoded 0.0) so friction/torque
      // feedforward reaches the gripper, same as arm joints. If the gripper feels HEAVIER
      // (not lighter) with friction comp on, the sign is flipped -> negate this.
  ...
  // LATENCY LEVER (pipelined CAN I/O): feedback from these MIT commands is drained at the
  // top of the NEXT read() (see read()), so no recv here. To REVERT: uncomment below and
  // restore refresh_all() in read().
