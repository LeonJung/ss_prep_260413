...
## §6. 마찰 Fv/Fo 조사 결론 (2026-07) — 미확정, 재식별 필요
포팅 후 openarmx_teleop 150Hz 안정(kp80 쓸만) 확인 뒤, 남은 뻑뻑함 레버로 마찰 Fv(점성)/Fo(오프셋)를 조사.

### 마찰 모델 (enactic 코드 = 우리와 동일)
`τ_fric(ω) = Fo + Fv·ω + Fc·tanh(0.1·k·ω)`
- **Fc** = Coulomb(건마찰) 크기 [Nm] — plateau 높이(최대 건마찰).
- **k** = tanh **경사(0속도 부근 급함 정도)**. `tanh(0.1·k·ω)`는 ω=0에서 0(정지시 마찰0, 매끈), ω↑에서 ±1로 포화(±Fc).
  - **knee 속도 ≈ 10/k rad/s** (여기서 마찰이 ~76% 도달). k↑=급격(이상적 sign에 가까움), k↓=완만.
  - tanh 쓰는 이유 = 이상적 Coulomb는 `Fc·sign(ω)`(ω=0 불연속) → 정지부근 속도노이즈에 **부호 튐(chatter)**. tanh는 매끈한 근사 → chatter 방지. k가 "얼마나 sign에 가깝나"를 정함.
- **Fv** = 점성마찰(속도 비례) [Nm·s/rad]. **Fo** = 상수 오프셋/바이어스 [Nm].

### 옛 데이터 4파라미터 재적합 결과
- **Fv = 0.000 (14관절 전부, 하한).** Fo는 일부 큼(leader J2 −0.85, follower J1 −1.0)이나 **중력잔차/데이터 비대칭
  아티팩트** → 적용 시 상수 토크 바이어스=팔 드리프트=위험. R² 0.09~0.55(낮음, Fc만 robust).
- ⚠ **"Fv=0이 물리적 확정"은 과한 단정이었음(정정).** DM(enactic)과 Robstride는 **같은 QDD 계열**(openarmx가
  DM→RS 1:1 대체) → "DM이 점성 많다"는 근거 없음(철회). enactic Fv≠0은 그들 식별 데이터(넓은 속도범위/깨끗)가
  작은 항을 잡은 것일 가능성. 우리 Fv=0은 **좁은 속도범위(0.1~1.2) + Robstride 토크추정 노이즈** 때문일 수 있음.

### 운영자 물리 관찰 (중요) — 무전원 back-EMF
- 무전원에서 J1/J2를 **빠르게 돌릴수록 저항↑** = 전자기 속도의존 감쇠. 두 원천: (1) **와전류/철손**(상 열려도 발생),
  (2) **back-EMF 제동**(상이 드라이버 통해 연결 시, ∝ω). 큰 모터(RS04, J1/J2)에서 Ke↑ → 제일 세게 = 일관.
- **핵심 구분**: **무전원**엔 back-EMF 제동이 그대로 느껴지나, **전원 운전(FOC 전류제어)** 중엔 **드라이버가
  back-EMF를 전압으로 보상** → follower 마찰로는 거의 안 남음. → 우리 powered 적합 Fv≈0과 **모순 아님**.
- 단 **철손은 운전 중에도 남는 진짜 속도의존 드래그** → **작은 Fv 실재 가능**. 그리고 고속에선 back-EMF가 전압한계
  먹어 **최대토크 하락**(마찰 아닌 별개 요인).

### 결론 / 다음
- **Fv/Fo 미확정.** 옛 데이터로는 못 뽑음(Fv=0, Fo=아티팩트). 물리적으로도 상당부는 무전원 한정 back-EMF.
- **확정법**: 전원 인가 상태에서 **더 넓은 속도범위(최대 운전속도까지) 재식별** → `fric(ω)` 선형상승=진짜 Fv(보상가치),
  평평=Coulomb 지배 확정. (P6에서 재식별 도구 필요 시 개발.)
- 지금 우리 모델(Coulomb Fc·tanh)은 무난. 뻑뻑함 다른 레버 = 관절별 kp(무료) / friction_scale 과보상(위험 주의) / HS-USB(rate).
