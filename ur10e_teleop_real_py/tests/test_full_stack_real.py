...
LOG_LEADER   = '/tmp/fullstack_real_leader.log'
LOG_FOLLOWER = '/tmp/fullstack_real_follower.log'
...
def _kill_stack():
    subprocess.run(['pkill', '-9', '-f', 'leader_real_node|follower_real_node'],
...
def phase0_setup():
    ...
    got_l = _wait_for_data('leader', timeout=8.0)
    got_f = _wait_for_data('follower', timeout=8.0)
...
def phase3_tracking():
    ...
    lsim = subprocess.Popen(
        ...
        stdout=open(LOG_LEADER, 'w'), stderr=subprocess.STDOUT)
    _stack['lsim'] = lsim
    _wait_for_data('leader', timeout=5.0)
    time.sleep(0.3)

    assert max_err[0] < 0.50, f'joint[0] err {max_err[0]:.3f} > 0.50'
    assert max_err[2] < 0.50, f'joint[2] err {max_err[2]:.3f} > 0.50'
...
def phase5_pause():
    ...
    assert dr_f < 0.05, f'follower drifted {dr_f:.4f}'
...
def phase6_homing():
    ...
    assert err_l < 0.05, f'leader home err {err_l:.4f}'
    assert err_f < 0.05, f'follower home err {err_f:.4f}'
