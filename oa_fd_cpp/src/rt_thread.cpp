// rt_thread.cpp — ported verbatim from ur10e_teleop_real_cpp (namespace oa_fd).
...
bool init_rt_thread(const RTConfig& cfg) {
  ...
  if (rc != 0) {
    std::fprintf(stderr,
      ...
      "  Hint: sudo setcap cap_sys_nice+ep,cap_ipc_lock+ep <binary>\n",
...
std::string JitterTracker::log_line(const std::string& prefix) {
  ...
  if (count_ == 0) { os << prefix << " jitter: (no samples)"; return os.str(); }
