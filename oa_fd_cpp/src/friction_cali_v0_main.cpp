// friction_cali_v0_main.cpp — oa_friction_cali_v0 (executable: oa_friction_cali_v0)
// ============================================================================
// A/B BASELINE: friction_cali from commit eab5a3d — the version from BEFORE any
// follower-arm calibration work (the era the leader friction cali ran "엄청
// 부드럽게"). Bent base (q4=0.3), full right mirror, clamp-to-sweep-ranges,
// both-direction linear sweep, KP/KD unchanged. NO edits of mine. Run next to
// the current `oa_friction_cali` on the SAME arm/setup: if THIS is smooth and
// current pops -> code regression in the cali main; if BOTH pop -> environment
// /hardware (it links the CURRENT driver lib, so it only isolates the main).
